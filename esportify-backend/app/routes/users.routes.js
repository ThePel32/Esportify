...
router.post('/signup', (req, res, next) => {
    console.log('[ROUTES] /api/users/signup -> auth.controller');
    return auth.signup(req, res, next);
});

router.post('/login', (req, res, next) => {
    console.log('[ROUTES] /api/users/login -> auth.controller');
    return auth.login(req, res, next);
});

router.post('/refresh-token', verifyToken, (req, res, next) => {
    console.log('[ROUTES] /api/users/refresh-token -> auth.controller');
    return auth.refreshToken(req, res, next);
});

router.get('/profile', verifyToken, (req, res, next) => {
    console.log('[ROUTES] /api/users/profile -> auth.controller');
